...
def cont_bottomUp(n: int) -> Tuple[int, Optional[List[str]]]: # metodo bottom-up 
    ...
    dp = [0] * (n + 1) # inicializa o array com zeros
    ...
    for x in range(2, n + 1): #cada val de x de 2 a n
        ...
        op[x] = '-1'             # a operacao usada foi decrementadaa
...
def main():     # funcao principal que implementa a interface de linha de comando    
    ...
    parser = argparse.ArgumentParser( # cria o parser de argumentos
        description='=== contagem regressiva da nasa ==== - encontra o menor numero de operacoes para reduzir n ate 1'
    ) 
    ...
    parser.add_argument(  # argumento opcional (metodo a ser usado)  
        ...
        help='metodos a serem usados: rec = recursivo, memo = memoizacao, dp = bottom-up'  #mensagem de ajuda 
    )
    ...
    try:
        if args.method == 'rec': #   metodo recursivo simples
            if args.n > 20:         # limitar para evitar stack overflow
                print("aviso: n muito grande para metodo recursivo simples. use --method memo ou dp") #mensagem de aviso
                sys.exit(1)
        ...
        print("erro: stack overflow. use um metodo nao-recursivo (--method dp)", file=sys.stderr) #mensagem de erro
        sys.exit(1)
